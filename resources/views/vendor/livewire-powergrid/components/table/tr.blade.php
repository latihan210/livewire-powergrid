...
<tr class="{{ theme_style($theme, 'table.header.tr') }}">
    @if ($loading)
    <td class="{{ theme_style($theme, 'table.body.tbodyEmpty') }}" colspan="999">
        @if ($loadingComponent)
    ...
    <th scope="col" class="{{ theme_style($theme, 'table.header.th') }}" wire:key="show-collapse-{{ $tableName }}">
    </th>
    @endif

    @isset($setUp['responsive'])
    <th fixed x-show="hasHiddenElements" class="{{ theme_style($theme, 'table.header.th') }}">
    </th>
    @endisset

    @if ($radio)
    <th class="{{ theme_style($theme, 'table.header.th') }}">
    </th>
    @endif
    ...
    <x-livewire-powergrid::cols wire:key="cols-{{ data_get($column, 'field') }} }}" :$column :$theme :$enabledFilters />
    @endforeach
    ...
    <th @if ($isActionFixedOnResponsive) fixed @endif
        ...
        scope="col" colspan="999" wire:key="{{ md5('actions') }}">
